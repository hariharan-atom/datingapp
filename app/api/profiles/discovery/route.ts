  ...
  try {
    ...
    const [
      ...
      admin
        ...
        .in("action", ["pass", "hide"]),
      admin.from("likes").select("target_user_id").eq("actor_user_id", user.id),
      admin
        .from("blocks")
    ...
    const excluded = new Set<string>([
      user.id,
      ...(profileActionsResult.data ?? []).map(
        (row) => row.target_user_id as string,
      ),
      ...(likesResult.data ?? []).map((row) => row.target_user_id as string),
